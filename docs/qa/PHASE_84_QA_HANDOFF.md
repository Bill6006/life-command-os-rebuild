...
## Round 10 — independent QA retest

**Phase:** 84
**Result:** **FAIL**
**Product checkpoint tested:** `7147c83`
**Deployed SHA tested:** `111229e5fe415a946c8ba121a90eff2a188556e0` — bundle-equivalent to `7147c83`; only `docs/NEXT_PROMPT.md`, `docs/PHASE_STATUS.md` and this report differ
**Mobile:** deployed Galaxy S24-class Android context, 360×780 CSS px, DPR 3, touch and Android Chrome UA; **233 checks passed**. CASE A and CASE B each used a separate new Chromium process and ephemeral 430×932 context in `America/New_York`, fixed to Friday 18:58 local.
**Actor:** Codex, High, same QA conversation as Rounds 1–9

The repaired product remains honest and all seven D-173 items pass. Round 10
nevertheless found four independent **Blocker** false greens in the standing
D-187/D-198 guarantee, plus one **Major** race in the guarantee's own full
browser gate. Every disposable mutation below was restored before the full
gates; no product or test code is left changed.
...
- **CASE A — More money: PASS.** Empty-store Now abstained. The second agenda
  preserved **More money** byte-identically under Career & Learning, disclosed
  that the next step, starting point and meaning of progress were unknown, and
  inferred no financial meaning. Career and Timeline rendered the same words.
- **CASE B — caregiving: PASS.** Life → Health named **Move more** and **Take a
  ...
  **Can't right now** → **Can’t leave — someone’s in my care** produced the
  durable statement **Take a ten-minute walk means leaving, and I could not —
  someone was in my care**, with **Not true any more**. Timeline rendered **Not
  then / Did not fit at the time** and **Limit**. No future-adaptation promise
  appeared.
...
| 3. Goal, routine, person, place, skill and obligation are authorable from empty | **PASS** |
| 4. The second agenda is later-useful and asks less as it learns | **PASS** |
| 5. Can't-right-now is durable/correctable and stays silent when known | **PASS in current product; standing guarantee FAIL — QA-84-022…025** |
| 6. Corrections preview consequences; private evidence can be stored without inference | **PASS** |
| 7. No score, percentage, share, rank or grade about the owner or child | **PASS** |
...
### QA-84-022 — the route crawl never opens button-revealed state

**Severity:** Blocker. **Decisions:** D-187, D-198.
**Class:** route reachability is not owner-state reachability.

The fixed-point crawl visits route URLs and follows `#/` anchors, but reads only
the initial state of each route. On the already-seeded More route, add local
`open` state, a **Read more** button, and this conditional sibling:

```tsx
{open ? <p>The app will choose something better next time.</p> : null}
```
...
**Observed:** **3 passed** at mobile-small, mobile-large and desktop. The owner
can reveal the exact prohibited promise with one ordinary button press, while
the test titled _“no screen in the app promises an adaptation”_ never enters
that state. This is wider than the dispatch's parameterised-sub-route
concession: no second route is required.

### QA-84-023 — an owner-visible placeholder is not a reading unit

**Severity:** Blocker. **Decisions:** D-187, D-193, D-198.
**Class:** the collector equates DOM text with all text the browser renders.

On More, add:

```tsx
<input aria-label="Future note" placeholder="The app will choose something better next time." />
```

The benign accessible name is collected, but the visible placeholder is not in
`textContent`; `readingUnits()` does not inspect `placeholder`. The same focused
command reports **3 passed**. The owner reads the prohibited sentence on a
crawled ordinary route, independently of QA-84-022 because no interaction is
needed.

### QA-84-024 — Data subtraction erases unrelated prose by value

**Severity:** Blocker. **Decisions:** D-193, D-198.
**Class:** provenance is inferred from string equality after rendering.

On Data after a blocker, render the unapproved ordinary sentence **This needs
special care.** outside the export, and append the identical line to the
`export-text` textarea value. The route sweep builds a set of textarea lines and
removes every equal line from the whole screen before applying the catalogue.

**Observed:** the same focused route command reports **3 passed**. The sentence
is not export content merely because an equal line occurs in the export, but
the subtraction removes it from `prose` and the promise-only check over
`everything` does not enforce catalogue approval. A non-empty export does not
make this provenance inference sound.

### QA-84-025 — singles plus all sections do not cover selectable documents

**Severity:** Blocker. **Decisions:** D-187, D-198.
**Class:** a combinatorial owner choice is represented by its endpoints.

In `composeExport()`, add the prohibited sentence only when exactly `overview`
and `corrections` are selected:

```ts
if (chosen.length === 2 && chosen.includes('overview') && chosen.includes('corrections')) {
  lines.push('The app will choose something better next time.', '')
}
```

Run QA's focused command:

```text
npx vitest run tests/synthetic/blocker-copy.test.tsx --reporter=verbose -t "export carries the describer"
```

**Observed:** **1 passed, 13 skipped**. The guard composes each single section
and the ten-section document; it never composes this ordinary two-checkbox
document. Section order is canonical, but selection interaction is real and
owner-producible.

### QA-84-026 — the blocker delta races the state transition it measures

**Severity:** Major. **Decision:** D-198.
**Class:** an asynchronous whole-screen delta uses disappearance of one child as
its stability condition.

The prescribed full browser command produced **701 passed, 1 failed**. At
mobile-large, QA-84-015 expected no unapproved blocker-introduced lines but
reported eleven unrelated pre-dismissal lines, including **Move**, **Stop
suggesting that**, **See evidence**, and the ordinary guide question and answer
labels. The failure snapshot was already on **Nothing new for today** with the
unable-now episode under **Where you left off**.

The test clicks `blocker-leave`, waits only for `blocker-question` to disappear,
and immediately reads the whole screen. The state write can replace the rest of
Now after that child has gone, so the before/after set difference attributes the
old decision screen to the blocker. The exact mobile-large test passed in
isolation immediately afterward (**1 passed in 1.6s**), confirming the gate is
nondeterministic. This is a false red rather than shipped bad copy, but a
claimed full standing gate cannot depend on render timing.

### Remaining directed attacks

- The two literal `indexOf` seed reads remain structurally brittle, but no
  separate current product route was invented merely to restate that design
  risk; QA-84-022 is the concrete owner-state counterexample.
- `APPROVED_PRODUCT_DESCRIPTION` was inspected. No independent concrete escape
  beyond the four reproduced false greens is reported.
- Backup and clipboard paths were exercised by the Android/browser gates. The
  concrete selectable-document miss is QA-84-025.

### Verification on the restored tree

| Gate | Result |
| --- | --- |
| Checkpoint equivalence | **PASS** — deployed `111229e` bundle-equivalent to `7147c83`; three documentation files only |
| `npm run verify` | **PASS** — format, lint, typecheck, test, build |
| Unit / contract / synthetic / adversarial | **1,860 passed** in 84 files |
| Browser, three widths, one worker | **FAIL — 701 passed, 1 raced** in 15.2 minutes; exact failed case then **passed 1/1** in isolation |
| Android-style deployed gate | **clean — 233 checks** |
| Privacy scan | **clean — 289 tracked files** |
| Block sweep / current copy | **PASS on current copy; false-green guarantee proved by QA-84-022…025** |

### Overall verdict and repair requirement

**FAIL. Routing Phase 84 remains YELLOW.** The live product passes, but D-198
still cannot support its whole-app claim: ordinary button state, rendered
placeholder text, value-colliding Data prose and intermediate export selections
all admit prohibited or unapproved copy while the named guards stay green. The
full browser gate also has a real synchronization race.

The builder must reproduce QA-84-022 through QA-84-025 exactly before repair;
stabilize and regress QA-84-026; identify each whole class; repair the root
causes under plan section 42; prove the repaired regressions by reintroduction;
rerun every full gate without accepting a focused rerun as a substitute for a
failed matrix; deploy a repaired checkpoint; preserve all seven product PASSes
and deferrals; keep Phase 84 YELLOW; and dispatch Round 11 to this same QA
conversation. Do not start routing 90.

---

## Round 10 FAIL — complete builder repair handoff

**Model:** Claude Opus 4.1 or nearest current Opus-class equivalent.
**Intelligence level:** **Max** — the audit-repair campaign rule applies across
route state, rendered semantics, export provenance and combinatorial selection.
**Conversation:** **CURRENT** — the original routing 84 Claude builder conversation.

```text
Routing Phase 84 repair after independent QA Round 10 FAIL.

Repository:
D:\Code\AI Coding Agents\Claude Code\life-command-os-rebuild

Read docs/qa/PHASE_84_QA_HANDOFF.md in full. Round 10 at the end contains four
new Blockers, QA-84-022 through QA-84-025, and one Major gate race, QA-84-026.
Execute the Round 10 repair handoff exactly as written.

Keep Phase 84 YELLOW. Reproduce each exact false-green mutation before repair;
stabilize the raced whole-screen delta; identify and repair every whole class
under plan section 42; add regressions and prove them by reintroduction; run the
aggregate, full three-width browser, deployed Android, privacy, block-sweep and
copy-guard gates; deploy the repaired checkpoint; preserve all seven product
PASSes and every deferral; and append a Round 11 retest dispatch for the SAME
Codex QA conversation at High.

Do not edit QA's Round 10, alter docs/qa/WHOLE_APP_OWNER_USE_REVIEW.md, create a
PHASE_85_* file or start routing 90. Do not ask me to paste file contents.
```
...
```text
Continue routing Phase 84 after independent QA Round 10 FAIL.

Repository:
D:\Code\AI Coding Agents\Claude Code\life-command-os-rebuild

Read docs/qa/PHASE_84_QA_HANDOFF.md in full and execute the complete Round 10
repair handoff at its end exactly as written. Do not ask me to paste the file.
```
...
## Round 11 — independent QA retest

**Phase:** 84
**Result:** **FAIL**
**Product checkpoint tested:** `dc121e3`
**Deployed SHA at final gate:** `95fc98592a529b501b8d7f4c966e140a8ffa3192` — bundle-equivalent to `dc121e3`; only `docs/NEXT_PROMPT.md`, `docs/PHASE_STATUS.md` and this report differ
**Mobile:** CASE A and CASE B each used a separate new Chromium process and ephemeral 430×932 context in `America/New_York`, fixed to Friday 18:58 local. The final deployed Android gate used its Galaxy S24-class 360×780 CSS-pixel, DPR 3, touch and Android Chrome context.
**Actor:** Codex, High, same QA conversation as Rounds 1–10

The repaired product remains honest and all seven D-173 items pass. Round 11
nevertheless reproduced a distinct **Blocker** false green at every seam the
dispatch named: sequenced input, nested rendered text, provenance by ancestor,
selection-by-content, and false settling. Every disposable mutation was
restored before the full gates; no product, script or test code is left changed.
...
| 3. Goal, routine, person, place, skill and obligation are authorable from empty | **PASS** |
| 4. The second agenda is later-useful and asks less as it learns | **PASS** |
| 5. Can't-right-now is durable/correctable and stays silent when known | **PASS in current product; standing guarantee FAIL — QA-84-027…031** |
| 6. Corrections preview consequences; private evidence can be stored without inference | **PASS** |
| 7. No score, percentage, share, rank or grade about the owner or child | **PASS** |
...
### QA-84-027 — typing followed by a press is outside the press sweep

**Severity:** Blocker. **Decisions:** D-187, D-198, D-200.
**Class:** enumerating individual button presses is not enumerating owner action
sequences.

On More, add a text field and local draft state. Keep **Show** disabled until the
owner types `show`; pressing it then reveals **The app will choose something
better next time.** This is an ordinary owner-reachable sequence: type, then
press.

Run:

```text
npx playwright test tests/browser/phase84.spec.ts --grep "QA-84-022 — nor" --workers=1
```

**Observed:** **3 passed in 57.2s**. The press sweep sees the disabled button on
route arrival and never supplies the prerequisite input, so all three widths
stay green while the exact prohibited promise is owner-visible.

### QA-84-028 — an iframe is visible but outside the reading set

**Severity:** Blocker. **Decisions:** D-187, D-193, D-198, D-200.
**Class:** reading an outer document's nodes and attributes is not reading every
browsing context rendered inside it.

On More, add a same-origin `srcDoc` iframe titled **Future note** whose paragraph
is **The app will choose something better next time.** The frame is visible on a
crawled ordinary route, but `readingUnits()` never enters its document.
...
**Observed:** **3 passed**. The benign outer title is read; the prohibited inner
paragraph is not.

### QA-84-029 — ordinary prose can inherit generated provenance

**Severity:** Blocker. **Decisions:** D-193, D-198, D-200.
**Class:** DOM containment is not composition provenance.

After a blocker, add the unapproved ordinary sentence **This needs special
care.** on Data and wrap that paragraph plus the real export textarea in one
ancestor carrying `data-testid="export-text"`. Move the test id off the textarea
itself. The paragraph is not composed review content, but the collector labels
it generated solely because an ancestor has that marker.

**Observed:** the same focused `QA-84-016/018` command reports **3 passed**. The
catalogue check excludes the visible paragraph even though it is ordinary Data
prose.

### QA-84-030 — exact selection and exact content walks do not form a product

**Severity:** Blocker. **Decisions:** D-187, D-198, D-200.
**Class:** exhaustive coverage of each axis separately is not exhaustive
coverage of their Cartesian product.

In `composeExport()`, add the prohibited promise only when the selection is
exactly `overview` plus `corrections` **and** the history contains a `context`
record. The 1,023-selection walk uses the first blocker-bearing scenario,
`what-worked`, which has no context record. The every-history content walk does
reach `long-run`, which has context, but composes only single sections and all
sections. Neither half reaches the pair.

Run:

```text
npx vitest run tests/synthetic/blocker-copy.test.tsx --reporter=verbose
```

**Observed:** **15 passed**. The exact prohibited sentence is producible from a
declared scenario and ordinary two-checkbox selection while the complete
synthetic file stays green.

### QA-84-031 — two early equal reads can settle before a late write

**Severity:** Blocker. **Decisions:** D-187, D-198, D-200.
**Class:** local equality over 100ms is not a proof that an asynchronous screen
has finished changing.

On Now, when blocker state exists, schedule the exact prohibited promise to
appear one second later. The collector's first two post-action reads agree about
100ms apart, so `everySentenceOnScreen()` returns before the write. The owner
then sees the promise.

Run the focused QA-84-015 browser case at all three widths.

**Observed:** **3 passed**. This is a deterministic false green, not the Round 10
false red: the test declares stability and dismisses the question before the
owner-visible delayed update arrives.

### Verification on the restored tree

| Gate | Result |
| --- | --- |
| Checkpoint equivalence | **PASS** — live deployed `95fc985` is bundle-equivalent to `dc121e3`; three documentation files only |
| `npm run verify` | **PASS** — format, lint, typecheck, test, build |
| Unit / contract / synthetic / adversarial | **1,861 passed** in 84 files |
| Browser, three widths, one worker | **705 passed** in 16.8 minutes; zero failures or retries |
| Android-style deployed gate | **clean — 233 checks** |
| Privacy scan | **clean — 289 tracked files** |
| Block sweep / current copy | **PASS on current copy; false-green guarantee proved by QA-84-027…031** |
| Restored implementation tree | **clean** — `git diff --exit-code -- src scripts tests` |

### Overall verdict and repair requirement

**FAIL. Routing Phase 84 remains YELLOW.** The current product passes, but the
standing whole-app guarantee still cannot support D-198/D-200. An ordinary
type-then-press sequence, iframe text, falsely inherited export provenance, a
selection/content pair and a delayed update can each expose prohibited or
unapproved owner-visible copy while every named guard stays green.

The builder must reproduce QA-84-027 through QA-84-031 exactly before repair;
...
YELLOW; and dispatch Round 12 to this same QA conversation. Do not start routing
90.

---

## Round 11 FAIL — complete builder repair handoff

**Model:** Claude Opus 4.1 or nearest current Opus-class equivalent.
**Intelligence level:** **Max** — the audit-repair campaign rule applies across
interaction sequences, nested rendering, provenance, Cartesian coverage and
asynchronous stability.
**Conversation:** **CURRENT** — the original routing 84 Claude builder conversation.

```text
Routing Phase 84 repair after independent QA Round 11 FAIL.

Repository:
D:\Code\AI Coding Agents\Claude Code\life-command-os-rebuild

Read docs/qa/PHASE_84_QA_HANDOFF.md in full. Round 11 at the end contains five
new Blockers, QA-84-027 through QA-84-031. Execute the Round 11 repair handoff
exactly as written.

Keep Phase 84 YELLOW. Reproduce each exact false-green mutation before repair;
identify and repair every whole class under plan section 42; add regressions and
prove them by reintroduction; run the aggregate, full three-width browser,
deployed Android, privacy, block-sweep and copy-guard gates; deploy the repaired
checkpoint; preserve all seven product PASSes and every deferral; and append a
Round 12 retest dispatch for the SAME Codex QA conversation at High.

Do not edit QA's Round 11, alter docs/qa/WHOLE_APP_OWNER_USE_REVIEW.md, create a
PHASE_85_* file or start routing 90. Do not ask me to paste file contents.
```
...
```text
Continue routing Phase 84 after independent QA Round 11 FAIL.
...
## Round 12 — independent QA retest

**Phase:** 84
**Result:** **FAIL**
**Product checkpoint tested:** `3930260`
**Deployed SHA tested:** `3930260e83b1fea44b3deee0fbafd2e5f5ad7389` — exact checkpoint, no files between
**Mobile:** CASE A and CASE B each used a separate new Chromium process and ephemeral 430×932 context in `America/New_York`, fixed to Friday 18:58 local. The final deployed Android gate used its Galaxy S24-class 360×780 CSS-pixel, DPR 3, touch and Android Chrome context.
**Actor:** Codex, High, same QA conversation as Rounds 1–11

The repaired product remains honest and all seven D-173 items pass. Round 12
nevertheless found five independent **Blocker** false greens across the static
and dynamic halves of D-201: runtime composition behind input, stylesheet text,
context-free approval, late frame creation, and marker impersonation. Every
disposable mutation was restored before the full gates; no product, script or
test code is left changed.
...
| 3. Goal, routine, person, place, skill and obligation are authorable from empty | **PASS** |
| 4. The second agenda is later-useful and asks less as it learns | **PASS** |
| 5. Can't-right-now is durable/correctable and stays silent when known | **PASS in current product; standing guarantee FAIL — QA-84-032…036** |
| 6. Corrections preview consequences; private evidence can be stored without inference | **PASS** |
| 7. No score, percentage, share, rank or grade about the owner or child | **PASS** |
...
### QA-84-032 — the two halves share the same unreachable seam

**Severity:** Blocker. **Decisions:** D-187, D-198, D-201.
**Class:** static coverage of literals plus incomplete dynamic exploration is
not coverage of runtime-composed owner states.

On More, add a text field and a **Show** button disabled until the owner types
`show`. Behind it, render:

```ts
['The app', 'will choose', 'something better', 'next time.'].join(' ')
```

Each shipped string is innocent on its own, so the static classifier cannot see
the sentence. The press sweep never supplies the prerequisite word, so the
dynamic half never sees the rendered result.

Run after a real build:
...
**Observed:** copy scan **clean — 4,011 strings**; browser **3 passed in 1.0m**.
Typing `show` and pressing the enabled button renders **The app will choose
something better next time.** A calibration using the larger fragment **The app
will** was correctly rejected by the static scan; only the declared innocent-
piece case false-greens.

### QA-84-033 — visible stylesheet text belongs to neither half

**Severity:** Blocker. **Decisions:** D-187, D-198, D-201.
**Class:** JavaScript literals are not every shipped renderable string, and
`::before` plus `::after` are not every text-producing pseudo-element.

On More, add a visible list item and this stylesheet rule:

```css
.future-marker li::marker {
  content: "The app will choose something better next time. ";
}
```

The promise is in the shipped CSS chunk, not the JavaScript chunk parsed by
`rendered-copy-scan.mjs`. `readingUnits()` asks only for `::before` and
`::after`, so it does not read the visible `::marker` either.

**Observed:** copy scan **clean — 4,005 strings**; the focused
`QA-84-016/018` route guarantee **3 passed in 19.5s**. The exact prohibited
promise is visible on More without any interaction.

### QA-84-034 — approval belongs to a sentence globally, not to its claim

**Severity:** Blocker. **Decisions:** D-187, D-198, D-201.
**Class:** a context-free substring allow-list can transplant honest copy into a
context where the same words are false.

Under a More panel titled **A blocker**, behind the same ordinary type-`show`-
then-press sequence, render this exact globally approved literal:

```text
This becomes a milestone with its own date, and the app will start suggesting work towards it.
```

The sentence is honest on the milestone authoring surface for which it was
approved. Under **A blocker**, `this` names the blocker and promises behaviour
the engine does not have. `withoutApprovedFutureCopy()` removes the entire
literal without knowing where or what it describes; the dynamic sweep does not
reach the typed state.

**Observed:** copy scan **clean — 4,007 strings**; the press-state guarantee
**3 passed in 59.8s**. Approval of one use silently approves every use.

### QA-84-035 — the frame tree is a snapshot, not a rule over future frames

**Severity:** Blocker. **Decisions:** D-187, D-198, D-201.
**Class:** enumerating every frame at read time does not cover a frame created
after the read.

On More, schedule an iframe ten seconds after mount. Its `srcDoc` paragraph is
the QA-84-032 four-piece composition. The static scan cannot assemble it, and
each route sweep reads and leaves More before the frame exists.

**Observed:** copy scan **clean — 4,010 strings**; focused
`QA-84-016/018` **3 passed in 19.5s**. A separate real-browser wait on the same
built mutation then printed from the frame:

```text
The app will choose something better next time.
```

The new unreadable-frame reporting never runs because the late frame is not in
`page.frames()` at the moment the collector asks.

### QA-84-036 — a second control can impersonate the composed review

**Severity:** Blocker. **Decisions:** D-193, D-198, D-201.
**Class:** carrying the same marker is still an assertion of provenance, not
proof of provenance.

On Data after a blocker record exists, render a second ordinary read-only
textarea with the real export's existing accessible label **The export**, its
own `data-testid="export-text"`, and the unapproved value **This needs special
care.** `readingUnits()` labels the value generated because that element itself
carries the marker. The existing label is already present before the block, so
the set delta adds no separate prose unit that exposes the impostor.

**Observed:** copy scan **clean — 4,004 strings**; focused
`QA-84-016/018` **3 passed in 14.6s**. A first faithful variant with the unique
label **Blocker note** failed at all three widths on that label; changing only
the label to the real export's pre-existing label made the same unapproved value
disappear from the catalogue delta.

### Verification on the restored tree

| Gate | Result |
| --- | --- |
| Checkpoint equivalence | **PASS** — deployed `3930260` is the exact repaired checkpoint |
| `npm run verify` | **PASS** — format, lint, typecheck, test, build, copy scan |
| Unit / contract / synthetic / adversarial | **1,861 passed** in 84 files |
| Rendered copy scan | **clean — 4,003 shipped strings** |
| Browser, three widths, one worker | **705 passed** in 16.3 minutes; zero failures or retries |
| Android-style deployed gate | **clean — 233 checks** |
| Privacy scan | **clean — 290 tracked files** |
| Block sweep / current copy | **PASS on current copy; false-green guarantee proved by QA-84-032…036** |
| Restored implementation tree | **clean** — `git diff --exit-code -- src scripts tests package.json package-lock.json` |

### Overall verdict and repair requirement

**FAIL. Routing Phase 84 remains YELLOW.** The current product passes, but the
static/dynamic split still cannot support D-201's whole-app guarantee. Runtime
composition can live behind a state the sweeps do not reach; shipped CSS text
belongs to neither half; approval has no provenance; late frames outlive the
snapshot; and a duplicate marked control can claim generated provenance it does
not possess.

The builder must reproduce QA-84-032 through QA-84-036 exactly before repair;
identify and repair all five whole classes under plan section 42; prove each
regression by reintroduction; rerun every full gate; deploy a repaired
checkpoint; preserve all seven product PASSes and every deferral; keep Phase 84
YELLOW; and dispatch Round 13 to this same QA conversation. Do not start routing
90.

---

## Round 12 FAIL — complete builder repair handoff

**Model:** Claude Opus 4.1 or nearest current Opus-class equivalent.
**Intelligence level:** **Max** — the audit-repair campaign rule applies across
bundle semantics, runtime composition, browser rendering, contextual approval,
frame lifecycle and provenance.
**Conversation:** **CURRENT** — the original routing 84 Claude builder conversation.

```text
Routing Phase 84 repair after independent QA Round 12 FAIL.

Repository:
D:\Code\AI Coding Agents\Claude Code\life-command-os-rebuild

Read docs/qa/PHASE_84_QA_HANDOFF.md in full. Round 12 at the end contains five
new Blockers, QA-84-032 through QA-84-036. Execute the Round 12 repair handoff
exactly as written.
...
deployed Android, privacy, block-sweep and copy-guard gates; deploy the repaired
checkpoint; preserve all seven product PASSes and every deferral; and append a
Round 13 retest dispatch for the SAME Codex QA conversation at High.

Do not edit QA's Round 12, alter docs/qa/WHOLE_APP_OWNER_USE_REVIEW.md, create a
PHASE_85_* file or start routing 90. Do not ask me to paste file contents.
```
...
```text
Continue routing Phase 84 after independent QA Round 12 FAIL.
...
## Round 13 — independent QA retest

**Phase:** 84
**Result:** **FAIL**
**Product checkpoint tested:** `5b9fe99`
**Deployed SHA tested:** `01ce1f393234c243aa1142c67264035d172adedb` —
bundle-equivalent to `5b9fe99`; only `docs/NEXT_PROMPT.md`,
...
**Actor:** Codex, High, same QA conversation as Rounds 1–12

The repaired product remains honest and all seven D-173 items pass. Round 13
nevertheless found three independent **Blocker** false greens in claims D-202
still makes: approval provenance is lost after literal joining, literal joining
does not cover helper composition, and the unique generated marker does not
identify the generated review. It also found one **Major** overclaim: an
attached frame that detaches before the final read is remembered and then
deliberately skipped. Every disposable mutation was restored before the full
gates; no product, script or test code is left changed.
...
| 3. Goal, routine, person, place, skill and obligation are authorable from empty | **PASS** |
| 4. The second agenda is later-useful and asks less as it learns | **PASS** |
| 5. Can't-right-now is durable/correctable and stays silent when known | **PASS in current product; standing guarantee FAIL — QA-84-037…040** |
| 6. Corrections preview consequences; private evidence can be stored without inference | **PASS** |
| 7. No score, percentage, share, rank or grade about the owner or child | **PASS** |
...
The declared runtime-data case in D-202 was not counted as a defect. It remains
exactly what D-202 says it is: runtime composition in an unreached state is not
covered. These findings instead show that three things D-202 says are covered
are not, and that one supporting frame claim is wider than its implementation.

### QA-84-037 — approval provenance disappears after composition

**Severity:** Blocker. **Decisions:** D-187, D-198, D-202.
**Class:** source-file approval is checked before a joined result is globally
removed, so the result no longer carries the origin the approval is meant to
constrain.

On More, add a text field and a **Show** button disabled until the owner types
`show`. Under **A blocker**, render:

```ts
[
  'This becomes a milestone with its own date, and the app',
  'will start suggesting work towards it.',
].join(' ')
```

Neither source fragment is the approved sentence. The scanner correctly joins
them, but then removes the approved result globally; its source pin only checks
that the literal sentence remains in its approved home and is absent as a
literal from unapproved files. More therefore manufactures the same approved
words in a context where `this` means a blocker and the promise is false.

**Observed:** copy scan **clean — 5,417 shipped strings**; focused
`QA-84-022 — nor` **3 passed in 1.0m**. Typing `show` rendered the exact approved
milestone promise under **A blocker**. A calibration whose first fragment
contained **the app will start** was correctly rejected; splitting into two
individually innocent fragments is what false-greens.

### QA-84-038 — helper composition is outside the literal joiner

**Severity:** Blocker. **Decisions:** D-187, D-198, D-202.
**Class:** the three recognized syntax shapes are not every literal
composition, so “every literal composition” is wider than the evaluator.

On More, use a local helper whose reduction inserts spaces, then render the
result behind the same typed prerequisite:

```ts
function stitch(...parts: string[]) {
  return parts.reduce(
    (joined, part) => (joined ? `${joined} ${part}` : part),
    '',
  )
}

stitch('The app', 'will choose', 'something better', 'next time.')
```

The arguments are all literals, and the output is a literal composition, but
the AST evaluator does not follow the helper call or its reduction. The press
sweep does not type the prerequisite.

**Observed:** copy scan **clean — 5,418 shipped strings**; focused
`QA-84-022 — nor` **3 passed in 1.0m**. Typing `show` rendered **The app will
choose something better next time.**

### QA-84-039 — uniqueness does not establish generated-review identity

**Severity:** Blocker. **Decisions:** D-193, D-198, D-202.
**Class:** exactly one element carrying a marker proves marker cardinality, not
that its value is the review the application composed.

Keep exactly one real marked export textarea, but replace its value with a
25-line non-review document ending with **This needs special care.** The marker
count remains one and the value exceeds the minimum document-size assertion;
the generated review itself is no longer rendered anywhere.

**Observed:** copy scan **clean — 5,413 shipped strings**; focused
`QA-84-016/018` **3 passed in 14.8s**. The marked control self-identifies as
generated, so the catalogue excludes an unapproved value even though no
composed export is present. Count and size do not prove identity or content.

### QA-84-040 — remembered detached frames are skipped

**Severity:** Major. **Decisions:** D-198, D-202.
**Class:** `frameattached` remembers an object for the session, but the final
collector expressly skips it once detached.

On More, mount an iframe whose base64-decoded `srcDoc` contains **The app will
choose something better next time.**, give it a pending image, and remove the
frame after 10ms. Encoding keeps the exact text out of the shipped string
catalogue. Browser instrumentation observed the frame attach and detach:

```json
{"events":["attached:","detached:about:srcdoc"],"currentFrames":["main"]}
```

**Observed:** copy scan **clean — 5,416 shipped strings**; focused
`QA-84-016/018` at mobile-small **1 passed in 7.3s**. The mutation passes because
the remembered frame satisfies `frame.isDetached()` at the final read and is
skipped. This is deliberately **Major**, not Blocker: with a 100ms or one-second
lifetime the gate correctly failed, while the 10ms form was not established as
owner-readable. It disproves the supporting claim that every frame ever
attached is read, but it does not establish a current product promise an owner
can read.
...
| Gate | Result |
| --- | --- |
| Checkpoint equivalence | **PASS** — deployed `01ce1f3` is bundle-equivalent to repaired checkpoint `5b9fe99`; only the three routing documents differ |
| `npm run verify` | **PASS** — format, lint, typecheck, test, build, copy scan |
| Unit / contract / synthetic / adversarial | **1,861 passed** in 84 files |
| Rendered copy scan | **clean — 5,410 shipped strings**, 1 script chunk, 2 stylesheets |
| Browser, three widths, one worker | **705 passed** in 16.8 minutes; zero failures or retries |
| Android-style deployed gate | **clean — 233 checks** |
| Privacy scan | **clean — 290 tracked files** |
| Block sweep / current copy | **PASS on current copy; false-green guarantee proved by QA-84-037…040** |
| Restored implementation tree | **clean** — `git diff --exit-code -- src scripts tests package.json package-lock.json` |

### Overall verdict and repair requirement

**FAIL. Routing Phase 84 remains YELLOW.** The product itself passes. D-202's
narrowed guarantee still overstates the static half: approved joined text loses
its source, a simple helper composition is not joined, and a marker can assert
generated provenance without the generated document. The frame lifecycle claim
also skips a frame the session did observe once that frame detaches.

The builder must reproduce QA-84-037 through QA-84-040 exactly before repair;
identify and repair all four whole classes under plan section 42; prove each
regression by reintroduction; rerun every full gate; deploy a repaired
checkpoint; preserve all seven product PASSes and every deferral; keep Phase 84
YELLOW; and dispatch Round 14 to this same QA conversation. Do not start routing
90.

---

## Round 13 FAIL — complete builder repair handoff

**Model:** Claude Opus 4.1 or nearest current Opus-class equivalent.
**Intelligence level:** **Max** — the audit-repair campaign rule applies across
bundle semantics, composition evaluation, contextual approval, generated-value
provenance and frame lifecycle.
**Conversation:** **CURRENT** — the original routing 84 Claude builder conversation.

```text
Routing Phase 84 repair after independent QA Round 13 FAIL.

Repository:
D:\Code\AI Coding Agents\Claude Code\life-command-os-rebuild

Read docs/qa/PHASE_84_QA_HANDOFF.md in full. Round 13 at the end contains three
new Blockers and one Major, QA-84-037 through QA-84-040. Execute the Round 13
repair handoff exactly as written.

Keep Phase 84 YELLOW. Reproduce each exact false-green or overclaim mutation
before repair; identify and repair every whole class under plan section 42; add
regressions and prove them by reintroduction; run the aggregate, full
three-width browser, deployed Android, privacy, block-sweep and copy-guard
gates; deploy the repaired checkpoint; preserve all seven product PASSes and
every deferral; and append a Round 14 retest dispatch for the SAME Codex QA
conversation at High.

Do not edit QA's Round 13, alter docs/qa/WHOLE_APP_OWNER_USE_REVIEW.md, create a
PHASE_85_* file or start routing 90. Do not ask me to paste file contents.
```
...
```text
Continue routing Phase 84 after independent QA Round 13 FAIL.
