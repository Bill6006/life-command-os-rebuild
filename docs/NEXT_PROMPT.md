...
3. docs/DECISION_LOG.md — decisions D-001 to D-059 and their reasons. D-059 is an owner decision made after Phase 3 closed and governs requirement 1 below.
...
1. How much a piece of evidence is worth depends on the source AND the concept it measures — never on the source alone (D-059). There is no standing hierarchy in which an explicit owner report always outweighs a derived, device or model one. A watch measuring hours slept may beat the owner's recollection of hours slept; a financial record of a balance may beat his estimate of it; a model's inference about how he feels should generally be weaker than him saying how he feels. Ranking by source alone is the same mistake section 8 forbids one layer down — "freshness is concept-specific, not one universal number of days" is the identical argument about a different property.

   Two things do not vary and are not reliability questions at all. Derived, inferred or model evidence must NEVER silently masquerade as explicit fact — a high-reliability inference is still an inference, and the four knowledge states exist so it cannot be read as anything else (D-014). And provenance stays visible wherever the evidence surfaces. Reliability decides how much a reading moves a belief; it never decides whether the reading can pass itself off as something it is not.

   Reuse the existing paths rather than creating another learning system. The learning weight is already `similarity * recencyFactor(days)` in learning.ts, and a reliability factor belongs there as one more term. `ConceptDefinition` in src/domain/concepts.ts already carries freshness, privacy and an ask policy per concept; a per-concept source-reliability entry is the same shape of thing and belongs beside them, with a conservative default per source that a concept may override where there is a reason to. PROVENANCE_SOURCES already contains 'derived', 'device' and 'model'.

   Read `knowledgeFromRecord` in src/memory/facts.ts before designing this. It already treats a `device` observation as `explicit` — the same standing as a self-report, not below it — which is the owner's point already embodied. It also gives every `derived` observation a flat confidence of 0.6 regardless of what is being measured, and that flat number is exactly what this requirement replaces. Do not write the sleep matcher against "derived is worth less": say what a watch or a morning self-report is worth for sleep hours specifically, and defend the number.

2. Learning traces must preserve and expose evidence provenance, so the owner can tell explicit owner reports from derived, inferred or device evidence. This is the "never masquerade" half of requirement 1 made checkable, and it holds regardless of how reliable a source turns out to be for a concept. Today LearningTrace.evidence is a bare RecordId[] and nothing carries the source, so "3 comparable results" could mean three things he said, three things the app concluded, or a mix — and he would have no way to tell before deciding whether to correct the belief. Do this before the first inferred outcome is written, not after.
...
- if inferred or derived outcomes are introduced, all four owner requirements above hold and each has a regression — including one proving that reliability is read per concept rather than per source, and one proving inferred evidence cannot be read as explicit
