...
| Relationship         | **Proved.** `node scripts/checkpoint-equivalence.mjs 0899f18 --deployed <build-info url>` found seven files changed between the checkpoint and the deployed `09cf875`, none bundle-relevant — six documents, a test file and the Android gate script. Never asserted as string equality (D-097): this repository redeploys on every push, including a documentation-only one. |
