...
| CI, including its own matrix run                               | PASS — runs 33784183228 and 33787629107, 849 each |
| Checkpoint equivalence                                         | PASS — read live twice; see _Checkpoint_ below    |
| Release integrity, from CI's own manifest artifact             | PASS — 8 files byte for byte, at both checkpoints |
| Android-style deployed gate                                    | PASS — 234 checks, against the deployed Preview   |
...
last one to change anything the browser downloads.** Everything committed after
it is documents plus the phone-gate script, which is why the equivalence holds
rather than merely happening to.

Equivalence was proved by reading the deployed `build-info.json` live, twice: at
`2618abe`, and again at `6486718` after the gate results were recorded. Both came
back bundle-equivalent to `b850dfc`, and release integrity was re-run at each
against that CI run's own manifest artifact. A later documents-only commit moves
the deployed SHA again without moving a byte the browser downloads, so the
property to check is that the diff from `b850dfc` is documents — not that the
deployed SHA equals any particular value.
...
list; Round 4 found that the catalogue which replaced it was closed over one
module rather than over the screen; Round 5 found that closing it over three
...
**And round 19 is where the owner closed the loop.** Rounds 15 to 19 produced
twenty findings, four per round, with no taper; nineteen were guard, scanner,
oracle or verifier findings and **none was an owner-visible product defect**.
Owner decision **D-210** defers those nineteen to
[`qa/INSTRUMENT_HARDENING_BACKLOG.md`](qa/INSTRUMENT_HARDENING_BACKLOG.md),
open and unedited, and keeps exactly one blocking: **QA-84-064**, which is about
release rather than about a detector. The deployment proved itself by reading a
commit identifier out of the served `build-info.json`, and a step between the
gate and the publish could change everything else without touching it. What is
served is now proved against a digest of what was verified (D-211). **Phase 84
GREEN, when QA declares it, means bounded product acceptance only.**

**And round 18 is where the guarantee stops covering one kind of thing while
claiming all of them.** The tie to `dist/` walked script chunks only, so a
plugin keyed on `isWrite` put a promise in the shipped stylesheet; `content: 'a'
'b' 'c'` was read as three values rather than the one the browser composes; and
an unplaced join — allowed because one straddling two modules belongs to
neither — also described a join a `renderChunk` plugin made out of nothing.
Round 18 then corrupted `composed` itself, so the field and the clipboard
delivered the same false document: two consumers of one object agree about
delivery, not about composition. The document is now composed again in the test
process, from the scenario's own history (D-208).

**And round 17 is where the build is asked for its own graph.** Five answers to
_which module produced these words_ have now been broken, and every one was a
second account of what the build did — reading `src`, the sourcemap, the map
corroborated against itself, and a walk of the relative imports, which Round 17
defeated with a **Vite alias**. Vite now runs in process, and Rollup's per-module
rendered code is the provenance: the shipped bytes, grouped by the module they
came from, by the tool that put them there. Round 17 also split a subject across
two pieces, so an opener may be assembled from adjacent ones; and it kept one
honest section while inventing nine, so the composed review is now compared
against what the copy control hands over (D-207).

**And round 16 is where corroboration leaves the account being corroborated.**
D-205 checked the sourcemap against the files on disk and against its own
positions — both statements by the build that made it. Round 16 changed no
shipped JavaScript and rewrote the map so the name, the content and the position
all agreed on the wrong module. Provenance is now answered from the
**repository**, by walking the app's relative imports; the map is an inventory
and a tripwire. Round 16 also dropped one argument of a helper to push a subject
away from its verb, so every pair of pieces that could carry a claim is tested;
and it made an invented review body vary by adding a counter, so one section
must now prove its body against what Timeline shows (D-206).
...
typed word. Literal composition, shipped CSS text, an approval with no sense of
place, an impostor marker and a late frame are all closed. **What is not closed
is now stated**: a sentence composed at runtime from data, in a state no sweep
reaches, is in neither half, and no wider sweep or cleverer parser decides it.
D-201's "whole-app" phrasing is superseded (D-202).
...
| Fact                    | Value                                                                                                                                                           |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Product checkpoint      | `d618588` — the bounded closeout under D-210: QA-84-064 repaired. **It changes no bundle byte**: the repair is in `scripts/`, the workflow and the build script |
| Round 19 checkpoint     | `16100ae` — the Round 18 repair, which Round 19 tested                                                                                                          |
| Round 18 checkpoint     | `6062756` — the Round 17 repair, which Round 18 tested                                                                                                          |
| Round 17 checkpoint     | `462dfe2` — the Round 16 repair, which Round 17 tested                                                                                                          |
| Round 16 checkpoint     | `9457b23` — the Round 15 repair, which Round 16 tested                                                                                                          |
...
## Independent QA — round 5, and the repair

**Result: FAIL, and the product half was clean for the third round running.**
Codex, at High, on the deployed `23ce35f` (bundle-equivalent to `f45214b`). All
seven acceptance items passed and QA-84-007 through QA-84-012 remain closed. The
full report is [`qa/PHASE_84_QA_HANDOFF.md`](qa/PHASE_84_QA_HANDOFF.md), Round 5,
committed unedited as `744dfa9` — **before** the repair, which is the ordering
Round 4 lost and this round put back.

The one finding was **QA-84-013**, and it is about the previous repair's own
words.

### Declaring a boundary is not closing one

Round 4's repair record named the history, Timeline, correction and export
renderers as a boundary it had **declared rather than closed**. Round 5 walked
through it. Changing the shared lifecycle frame for an `action-unable-now` to
_"The app will choose something better next time"_ put that promise in front of
the owner on four surfaces while five suites passed **431 / 431**. It was
...
`CanonicalRecord` — and all four of those surfaces render its one sentence.

### The repair — D-195

**The guarantee now has three halves**, each proved by the check that can reach
it: what `blockers.ts` **assembles**, walked through the scenario library; what
the surfaces **compose** in JSX, proved by rendering; and what a record **reads
as**, proved by describing one. The third covers all four surfaces at once.

**And the describers are enumerated from source**, which is the half that makes
it last. `recordTextFunctionsInSource()` returns every exported function in
`src/` taking a `CanonicalRecord` — thirteen. Five produce owner text and are
exercised over every record the blocker path writes; the other eight are named
in `NOT_OWNER_TEXT` with the reason each gives the owner no words. A fourth
describer fails until somebody classifies it.

**No product code changed.** The copy was honest throughout, which is why five
rounds of gates and an owner-use walk did not see it. What was defective was the
guarantee.
...
### The cold-store evidence Round 5 could not produce
...
was the right call**, and CASE A and CASE B from a fresh store are still owed.

There is a technique that needs neither clearing nor authorisation and this
